...
| `oidcConfig` _[api.v1alpha1.OIDCConfigRef](#apiv1alpha1oidcconfigref)_ | OIDCConfig defines OIDC authentication configuration for the proxy.<br />When set, incoming requests are validated against the configured OIDC provider.<br />When omitted, the proxy allows anonymous access (no authentication required). |  | Optional: \{\} <br /> |
